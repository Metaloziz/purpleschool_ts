...
## 33. TS compiler. include and exclude files.
...
## 34. Вывод компиляции
    ...
    1) "outDir": "./build/"
        // Куда сохранять JS файлы

    2) "removeComments": true,

    3) "sourceMap": true, 
        // рядом с JS файлами появляются файлы для отслеживания исходников из TS
    
    4) "declaration": true,
        "declarationDir": "./build/types/",     

        // указываем место где сохранить отдельно все типы

# Generics

## 35. Ограничение generic

        T может наследоваться

## 36. Indexed Access Types

    К ключам в типах можно обращаться как к ключам объекта через имя индекса. На любую вложенность.

## 37. Conditional Types

    Типы могут быть динамическими. Смотри код

## 38. Mapped Types

    мы можем задать один тип всем ключам в другом типе типе через в одну строку. 
    В мапинге можно изменять обязательность(-+?) и свойства(readonly) ключей.
    Используется когда тип с подходящими для новоко типа ключами но разными 
    значениями. Из первого мы можем создать второй.

## 39. Template Literal Types

    Крутая вещь - смотри код
